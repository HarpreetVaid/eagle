...
[![PyPI - Python](https://img.shields.io/badge/python-v3.12+-blue.svg)](https://github.com/katanaml/eagle)
[![GitHub Stars](https://img.shields.io/github/stars/katanaml/eagle.svg)](https://github.com/katanaml/eagle/stargazers)
[![GitHub Issues](https://img.shields.io/github/issues/katanaml/eagle.svg)](https://github.com/katanaml/eagle/issues)
[![Current Version](https://img.shields.io/badge/version-0.4.4-green.svg)](https://github.com/katanaml/eagle)
[![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue.svg)](https://www.gnu.org/licenses/gpl-3.0)

**Structured data extraction and instruction calling with ML, LLM and Vision LLM**

<p align="center">
  <img width="300" height="300" src="https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/eagle_logo_5.png">
</p>

> **Acknowledgment**: Eagle is a specialized fork and restructuring of the **[Sparrow](https://github.com/katanaml/sparrow)** project by KatanaML. We extend our deepest gratitude to the original authors for their open-source foundation in structured data extraction.

<p align="center">
  <strong>🚀 <a href="https://eagle.katanaml.io">Try Eagle Online</a> | 📖 <a href="#-quickstart">Quick Start</a> | 🛠️ <a href="#️-installation">Installation</a> | 📚 <a href="#-examples">Examples</a> | 🤖 <a href="#-eagle-agent">Agents</a></strong>
</p>
...
### Try Online
Visit [eagle.katanaml.io](https://eagle.katanaml.io) for a live demo running on Mac Mini M4 Pro.

![Eagle UI](https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/eagle_ui.png)
...
🔧 **Pluggable Architecture**: Mix and match different pipelines (Eagle Parse, Instructor, Agents)  
...
![Eagle Architecture](https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/eagle_architecture.jpeg)

### Core Components

| Component | Purpose | Use Case |
|-----------|---------|----------|
| **[Eagle ML LLM](https://github.com/katanaml/eagle/tree/main/eagle-ml/llm)** | Main API engine | Document processing pipelines |
| **[Eagle Parse](https://github.com/katanaml/eagle/tree/main/eagle-data/parse)** | Vision LLM library | Structured JSON extraction |
| **[Eagle Agents](https://github.com/katanaml/eagle/tree/main/eagle-ml/agents)** | Workflow orchestration | Complex multi-step processing |
| **[Eagle OCR](https://github.com/katanaml/eagle/tree/main/eagle-data/ocr)** | Text recognition | OCR preprocessing |
| **[Eagle UI](https://github.com/katanaml/eagle/tree/main/eagle-ui/)** | Web interface | Interactive document processing |
...
# or .env_eagle_parse\Scripts\activate  # Windows

# 3. Install Eagle Parse pipeline
git clone https://github.com/katanaml/eagle.git
cd eagle/eagle-ml/llm
...
Before running `pip install -r requirements_eagle_parse.txt`, check your platform. If you are on macOS and want to run MLX backend, go to `requirements_eagle_parse.txt` and make sure `eagle-parse[mlx]` libary reference is defined. If you are running Eagle on Linux/Windows, make sure to use `eagle-parse` library reference, this will skip MLX related libraries.

### First Document Extraction

```bash
# Extract data from a bonds table
./eagle.sh '[{"instrument_name":"str", "valuation":0}]' \
...
2. **Virtual Environments**: Create separate environments for different pipelines:
   - `.env_eagle_parse` - for Eagle Parse (Vision LLM)
...
# Visit http://localhost:8002/api/v1/eagle-llm/docs
```
...
![Bank Statement](https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/bank_statement.png)
...
![Bonds Table](https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/bonds_table.png)
...
./eagle.sh '{"table": [{"description": "str", "latest_amount": 0, "previous_amount": 0}]}' \
  --pipeline "eagle-parse" \
  --options mlx \
  --options mlx-community/Qwen2.5-VL-72B-Instruct-4bit \
...
./eagle.sh "instruction: do arithmetic operation, payload: 2+2=" \
...
./eagle.sh "check if business entity Chapman, Kim and Green is invoice issuing party" 
  --pipeline "eagle-parse" 
  --instruction 
...
./eagle.sh assistant --pipeline "stocks" --query "Oracle"
```
...
```bash
./eagle.sh "<JSON_SCHEMA>" --pipeline "<PIPELINE>" [OPTIONS] --file-path "<FILE>"
```
...
| `--pipeline` | String | Pipeline to use | `eagle-parse` |
...
| `--instruction` | Boolean | Eagle query will be used as instruction | `--instruction` |
| `--validation` | Boolean | Eagle query will be used for field validation | `--validation` |
...
./eagle.sh '[{"required_field":"str", "optional_field":"str or null"}]' \
  --pipeline "eagle-parse" \
  --options mlx \
  --options mlx-community/Qwen2.5-VL-72B-Instruct-4bit \
...
# Field validation
./eagle.sh "tax_id,shipment_code,total_gross_worth" 
  --pipeline "eagle-parse" 
  --validation 
  --options mlx --options lmstudio-community/Mistral-Small-3.2-24B-Instruct-2506-8bit 
  --file-path "invoice_1.jpg"
...
# Multiple instances
python api.py --port 8002 &  # Eagle Parse
...
```bash
curl -X POST 'http://localhost:8002/api/v1/eagle-llm/inference' \
  -H 'Content-Type: multipart/form-data' \
  -F 'query=[{"field_name":"str", "amount":0}]' \
  -F 'pipeline=eagle-parse' \
...
```bash
curl -X POST 'http://localhost:8002/api/v1/eagle-llm/instruction-inference' \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  -d 'query=instruction: analyze data, payload: {...}' \
  -d 'pipeline=eagle-instructor' \
  -d 'options=mlx,mlx-community/Qwen2.5-VL-72B-Instruct-4bit'
```

### API Documentation

Visit `http://localhost:8002/api/v1/eagle-llm/docs` for interactive Swagger documentation.

![API Documentation](https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/eagle_api.png)

## 🤖 Eagle Agent

![Eagle Agents](https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/eagle_agent.png)
...
curl -X POST 'http://localhost:8001/api/v1/eagle-agents/execute/file' \
  -F 'agent_name=medical_prescriptions' \
  -F 'extraction_params={"eagle_key":"123456"}' \
  -F 'file=@prescription.pdf'
```

## 📊 Dashboard

Built-in analytics and monitoring dashboard at [eagle.katanaml.io](https://eagle.katanaml.io). This is part of Eagle UI, requires local Oracle Database 23ai Free. 

![Dashboard](https://github.com/katanaml/eagle/blob/main/eagle-ui/assets/eagle_ui_3.png)
...
| Feature | Eagle Parse | Eagle Instructor | Eagle Agents |
|---------|---------------|-------------------|----------------|
...
**Eagle Parse**: Use for structured data extraction from documents  
**Eagle Instructor**: Use for text analysis, summarization, Q&A  
**Eagle Agents**: Use for complex multi-step document processing workflows  
...
2. **🐛 Search Issues**: [GitHub Issues](https://github.com/katanaml/eagle/issues)  
...
[![Star History Chart](https://api.star-history.com/svg?repos=katanaml/eagle&type=Date)](https://star-history.com/#katanaml/eagle&Date)
...
<p align="center">
  <strong>⭐ Star us on GitHub if Eagle is useful for your projects!</strong><br>
  <a href="https://github.com/katanaml/eagle">github.com/katanaml/eagle</a>
</p>
