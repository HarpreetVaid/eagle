...
app = FastAPI(lifespan=lifespan,
              ...
              openapi_url="/api/v1/eagle-llm/openapi.json",
              docs_url="/api/v1/eagle-llm/docs")
...
@app.get("/")
def root():
    return {"message": "Eagle LLM API"}


def validate_key_from_config(config, eagle_key):
    """
    Validates and increments usage count for a eagle key using config.

    Args:
        config: Configuration object containing eagle keys
        eagle_key (str): The key to validate (must not be None or empty)
    ...
    """
    # Get all Eagle keys
    eagle_keys = config.get_eagle_keys()

    # Validate and log usage
    key_found = False
    for key_name, data in eagle_keys.items():
        if data['value'] == eagle_key:
            key_found = True
            ...
            if usage_count >= usage_limit:
                raise HTTPException(
                    status_code=403,
                    detail=f"Usage limit exceeded for key '{eagle_key}'. Allowed limit: {usage_limit}."
                )
...
@app.post("/api/v1/eagle-llm/inference", tags=["LLM Inference"])
async def inference(
        ...
        eagle_key: Annotated[Optional[str], Form()] = None,
    ...
    if protected_access:
        ...
        if not eagle_key:
            raise HTTPException(
                status_code=403,
                detail="Eagle key is required for protected access."
            )
        ...
        if use_database:
            ...
            is_valid = db_pool.validate_and_increment_key(eagle_key)
            ...
            validate_key_from_config(config, eagle_key)
...
@app.post("/api/v1/eagle-llm/instruction-inference", tags=["LLM Inference"])
...
# go to http://127.0.0.1:8000/api/v1/eagle-llm/docs to see the Swagger UI
