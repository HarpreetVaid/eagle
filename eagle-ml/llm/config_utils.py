...
class EagleConfig:
    """
    Unified configuration utility for Eagle services.
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            cls._instance = super(EagleConfig, cls).__new__(cls)
    ...
    def _create_default_config(self):
        ...
        default_config['settings'] = {
            ...
            'version': '0.5.5',
            'backend_url': 'http://localhost:8002/api/v1/eagle-llm/inference',
    ...
    def get_eagle_keys(self) -> Dict[str, Dict[str, Any]]:
        """
        Get all Eagle API keys from the configuration.
    ...
    def update_key_usage(self, key_name: str, new_count: int) -> None:
        ...
        if 'keys' in self._config and key_name in self.get_eagle_keys():
    ...
    def get_eagle_key_value(self, key_name: str) -> Optional[str]:
        """Get the value of a specific key"""
        keys = self.get_eagle_keys()
        if key_name in keys:
...
if __name__ == "__main__":
    ...
    print(f"Eagle Keys: {json.dumps(config.get_eagle_keys(), indent=2)}")