...
async def detect_doc_structure(input_data: Dict[str, Any], eagle_client: EagleClient) -> Dict:
    """
    ...
    Args:
        ...
        eagle_client: Instance of EagleClient for API calls
    ...
    return await eagle_client.extract_type_per_page_eagle(input_data)
...
async def split_document(input_data: Dict[str, Any], doc_structure: Dict) -> List[Dict]:
    ...
    Args:
        ...
        doc_structure: Document structure from Eagle API
...
async def process_adjudication_table(input_data: Dict[str, Any], page_data: Dict[str, Any], eagle_client: EagleClient) -> Dict:
    """
    Process pages of type 'adjudication_table' using Eagle API.
    ...
    try:
        ...
        result = await eagle_client.extract_data_eagle(
...
async def process_adjudication_details(input_data: Dict[str, Any], page_data: Dict[str, Any], eagle_client: EagleClient) -> Dict:
    """
    Process pages of type 'adjudication_details' using Eagle API.
...
async def extract_data(input_data: Dict[str, Any], pages: List[Dict], eagle_client: EagleClient) -> List:
    """
    ...
    for page in pages:
        ...
        if page_type == 'adjudication_table' or page_type == 'invoice_request_form':
            result = await process_adjudication_table(input_data, page, eagle_client)
        elif page_type == 'adjudication_details' or page_type == 'application_for_coverage' or page_type == 'patient_info':
            result = await process_adjudication_details(input_data, page, eagle_client)
        else:
...
class MedicalPrescriptionsAgent:
    """
    Agent for processing medical prescriptions using Eagle API.
    """
    ...
    async def execute(self, input_data: Dict) -> Dict:
        ...
        doc_structure = await detect_doc_structure(input_data, self.eagle_client)

        # Split document into pages
        pages = await split_document(input_data, doc_structure)

        # Process each page
        results = await extract_data(input_data, pages, self.eagle_client)
