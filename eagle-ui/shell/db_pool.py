...
def get_restricted_key(client_ip):
    """
    Call the obtain_eagle_key PL/SQL function to get a restricted key
    ...
    Returns:
        str or None: A eagle key if available, None if rate limited or error
    """
    ...
    try:
        ...
        cursor.execute(
            "BEGIN :result := obtain_eagle_key(:ip); END;",
            result=out_var,
            ip=client_ip
        )
        ...
        print(f"Error calling obtain_eagle_key: {str(e)}")
        return None
    finally:
        if connection:
...
def verify_key(key):
    """
    Verify if a provided Eagle key exists in the database and is enabled.

    Args:
        key (str): The Eagle key to verify
...
def get_inference_logs(period="1week"):
    ...
    excluding records with EAGLE_KEY_ID = 1 and COUNTRY_NAME that is 'Unknown' or NULL.
...
def get_unique_users_by_country(period="1week"):
    ...
    for a specified time period, excluding records with EAGLE_KEY_ID = 1
